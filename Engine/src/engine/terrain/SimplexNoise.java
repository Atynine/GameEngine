...
public class SimplexNoise {  // Simplex noise in 2D, 3D and 4D
    ...
    public static void seed(long seed){
        //Seeds the values to create a random terrain generation
        Random random = new Random(seed);
        perm = new short[512];
        for(int i = 0; i < 512; i++){
            perm[i] = (short) random.nextInt(256);
        }
        for(int i = 0; i < 512; i++){
            perm[i] = (short) random.nextInt(256);
        }
    ...
    public static float[][] generateOctavedSimplexNoise(int width, int height, int xOffset, int yOffset,int octaves, float roughness, float scale){
        float[][] totalNoise = new float[width][height];
        float layerFrequency = scale;
        float layerWeight = 1;
        float weightSum = 0;
        ...
        for (int octave = 0; octave < octaves; octave++) {
            ...
            for(int x = xOffset; x < width+xOffset; x++){
                for(int y = yOffset; y < height+yOffset; y++){
                    totalNoise[x-xOffset][y-yOffset] += (float) noise(x * layerFrequency,y * layerFrequency) * layerWeight;
                }
            }
